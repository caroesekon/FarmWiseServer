...
const getTeam = async (req, res) => {
  try {
    const team = await User.find({ farmId: req.farmId })
      .select('-password')
      .sort({ role: 1, name: 1 });
...
const addMember = async (req, res) => {
  try {
    ...
    logger.info('[Client Team] Added member', {
      userId: user._id,
      farmId: req.farmId,
      role,
    });
...
const removeMember = async (req, res) => {
  try {
    ...
    if (user.role === 'farmAdmin') {
      return res.status(400).json({
        success: false,
        message: 'Cannot remove the farm admin.',
      });
    }

    await user.deleteOne();

    logger.info('[Client Team] Removed member', {
      userId: user._id,
      farmId: req.farmId,
    });

...
const getCrops = async (req, res) => {
  try {
    ...
    const crops = await Crop.find(query).sort({ plantingDate: -1 }).skip(skip).limit(parseInt(limit));
    const total = await Crop.countDocuments(query);

    res.status(200).json({ success: true, count: crops.length, total, totalPages: Math.ceil(total / parseInt(limit)), currentPage: parseInt(page), data: crops });
  } catch (error) {
...
const addCrop = async (req, res) => {
  try {
    ...
    if (!fieldId || !cropType || !plantingDate) {
      return res.status(400).json({ success: false, message: 'Field, crop type, and planting date are required.' });
    }

    const crop = await Crop.create({ farmId: req.farmId, fieldId, cropType, variety, plantingDate, expectedHarvestDate, recordedBy: req.user.id, notes });
...
const updateCrop = async (req, res) => {
  try {
    const crop = await Crop.findOne({ _id: req.params.id, farmId: req.farmId });
    if (!crop) return res.status(404).json({ success: false, message: 'Crop not found.' });

    const { cropType, variety, plantingDate, expectedHarvestDate, status, notes } = req.body;
    ...
    logger.error('[Client Crop] Update failed', { error: error.message });
...
const harvestCrop = async (req, res) => {
  try {
    ...
    crop.yield = { quantity: yieldQuantity, unit: yieldUnit };
    ...
    res.status(500).json({ success: false, message: 'Failed to record harvest.' });
  }
};
...
const deleteCrop = async (req, res) => {
  try {
    const crop = await Crop.findOneAndDelete({ _id: req.params.id, farmId: req.farmId });
    if (!crop) return res.status(404).json({ success: false, message: 'Crop not found.' });

    logger.info('[Client Crop] Deleted', { cropId: crop._id });
    res.status(200).json({ success: true, message: 'Crop deleted.' });
  } catch (error) {
    logger.error('[Client Crop] Delete failed', { error: error.message });
    res.status(500).json({ success: false, message: 'Failed to delete crop.' });
  }
};

module.exports = { getCrops, addCrop, updateCrop, harvestCrop, deleteCrop };
...
const getFields = async (req, res) => {
  try {
    ...
    const fields = await Field.find(query).sort({ name: 1 }).skip(skip).limit(parseInt(limit));
    const total = await Field.countDocuments(query);

    res.status(200).json({ success: true, count: fields.length, total, totalPages: Math.ceil(total / parseInt(limit)), currentPage: parseInt(page), data: fields });
  } catch (error) {
...
const addField = async (req, res) => {
  try {
    ...
    const field = await Field.create({ farmId: req.farmId, name, size, sizeUnit, soilType, currentCrop, notes });
...
const updateField = async (req, res) => {
  try {
    const field = await Field.findOne({ _id: req.params.id, farmId: req.farmId });
...
const deleteField = async (req, res) => {
  try {
    const field = await Field.findOneAndDelete({ _id: req.params.id, farmId: req.farmId });
    if (!field) return res.status(404).json({ success: false, message: 'Field not found.' });

    await Crop.deleteMany({ fieldId: field._id });

    logger.info('[Client Field] Deleted', { fieldId: field._id });
    res.status(200).json({ success: true, message: 'Field and associated crops deleted.' });
  } catch (error) {
    logger.error('[Client Field] Delete failed', { error: error.message });
    res.status(500).json({ success: false, message: 'Failed to delete field.' });
  }
};

module.exports = { getFields, addField, updateField, deleteField };
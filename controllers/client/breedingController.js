  ...
  try {
    ...
    if (!animalId || !eventType) {
      return res.status(400).json({
        success: false,
        message: 'Animal ID and event type are required.',
      });
    }
    ...
    logger.info('[Client Breeding] Event added', {
      recordId: record._id,
      farmId: req.farmId,
      eventType,
    });
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to add breeding event.',
    });
  }
};
...
const updateBreedingEvent = async (req, res) => {
  try {
    const record = await Breeding.findOne({
      _id: req.params.id,
      farmId: req.farmId,
    });
    ...
    const { status, pregnancyStatus, calfCount, calfIds, notes } = req.body;
